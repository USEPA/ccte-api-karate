...
Feature: Feature file for validating responses of bioactivity Toxcast AOP resources
  ...
  Background:
    ...
    * def responseSchema = {id: "#present", toxcastAeid: '#present', entrezGeneId: '#present', eventNumber: '#present', eventLink: '#present', aopNumber: '#present', aopLink: '#present'}
  ...
  Scenario: Validating the response of the GET method for AOP search using toxcast aeid
    ...
    When method GET
    Then status 200
    And match response[0] == responseSchema
